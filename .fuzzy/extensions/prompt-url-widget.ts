import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@fuzzyos/fuzzy-code";
...
async function fetchGhMetadata(
	...
	try {
		const result = await fuzzy.exec("gh", args);
...
export default function promptUrlWidgetExtension(fuzzy: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = fuzzy.getSessionName()?.trim();
		if (!currentName) {
			fuzzy.setSessionName(desiredName);
			return;
		}
	...
	fuzzy.on("before_agent_start", async (event, ctx) => {
		...
		void fetchGhMetadata(fuzzy, match.kind, match.url).then((meta) => {
