...
import { Container, Key, matchesKey, type SelectItem, SelectList, Text } from "@fuzzyos/fuzzy-tui";
...
export default function (fuzzy: ExtensionAPI) {
	fuzzy.registerCommand("diff", {
		...
		handler: async (_args, ctx) => {
			...
			const result = await fuzzy.exec("git", ["status", "--porcelain"], { cwd: ctx.cwd });
			...
			const openSelected = async (fileInfo: FileInfo): Promise<void> => {
				try {
					...
					if (fileInfo.status === "?") {
						await fuzzy.exec("code", ["-g", fileInfo.file], { cwd: ctx.cwd });
						return;
					}

					const diffResult = await fuzzy.exec("git", ["difftool", "-y", "--tool=vscode", fileInfo.file], {
						cwd: ctx.cwd,
					});
