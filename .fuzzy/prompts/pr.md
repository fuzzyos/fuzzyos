...
4. Check for a changelog entry in the relevant `packages/*/CHANGELOG.md` files. Report whether an entry exists. If missing, state that a changelog entry is required before merge and that you will add it if the user decides to merge. Follow the changelog format rules in AGENTS.md. Verify:
   ...
   - External contributions include PR link and author: `Fixed foo ([#123](https://github.com/fuzzyos/fuzzyos/pull/123) by [@user](https://github.com/user))`
   - Breaking changes are in `### Breaking Changes`, not just `### Fixed`
5. Check if packages/fuzzy-code/README.md, packages/fuzzy-code/docs/*.md, packages/fuzzy-code/examples/**/*.md require modification. This is usually the case when existing features have been changed, or new features have been added.
