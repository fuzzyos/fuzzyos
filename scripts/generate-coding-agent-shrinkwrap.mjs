...
try {
	...
	if (checkOnly) {
		if (!existsSync(shrinkwrapPath)) {
			console.error("packages/fuzzy-code/npm-shrinkwrap.json is missing.");
			console.error("Run: npm run shrinkwrap:coding-agent");
			process.exit(1);
		}
		const current = readFileSync(shrinkwrapPath, "utf8");
		if (current !== content) {
			console.error("packages/fuzzy-code/npm-shrinkwrap.json is out of date.");
			console.error("Run: npm run shrinkwrap:coding-agent");
			process.exit(1);
		}
		console.log("packages/fuzzy-code/npm-shrinkwrap.json is up to date.");
	} else {
