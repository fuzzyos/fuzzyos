 ...
 *   --analyze      Spawn fuzzy subagents to analyze each transcript file
...
import { parseSessionEntries, type SessionMessageEntry } from "../packages/fuzzy-code/src/core/session-manager.js";
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("fuzzy", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn fuzzy: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".fuzzy/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn fuzzy subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".fuzzy/agent/AGENTS.md");
