#!/usr/bin/env bash
#
# Build fuzzy binaries for all platforms locally.
...
#
# Output:
#   packages/fuzzy-code/binaries/
#     fuzzy-darwin-arm64.tar.gz
#     fuzzy-darwin-x64.tar.gz
#     fuzzy-linux-x64.tar.gz
#     fuzzy-linux-arm64.tar.gz
#     fuzzy-windows-x64.zip
...
if [[ "$SKIP_DEPS" == "false" ]]; then
    ...
    npm install --no-save --force \
        @fuzzyos/clipboard-darwin-arm64@0.3.9 \
        @fuzzyos/clipboard-darwin-x64@0.3.9 \
        @fuzzyos/clipboard-linux-x64-gnu@0.3.9 \
        @fuzzyos/clipboard-linux-arm64-gnu@0.3.9 \
        @fuzzyos/clipboard-win32-x64-msvc@0.3.9 \
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/cli.js --outfile binaries/$platform/fuzzy.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/cli.js --outfile binaries/$platform/fuzzy
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating fuzzy-$platform.zip..."
        (cd $platform && zip -r ../fuzzy-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating fuzzy-$platform.tar.gz..."
        mv $platform fuzzy && tar -czf fuzzy-$platform.tar.gz fuzzy && mv fuzzy $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../fuzzy-$platform.zip)
    else
        tar -xzf fuzzy-$platform.tar.gz && mv fuzzy $platform
    fi
done

echo ""
echo "==> Build complete!"
echo "Archives available in packages/fuzzy-code/binaries/"
ls -lh *.tar.gz *.zip 2>/dev/null || true
echo ""
echo "Extracted directories for testing:"
for platform in "${PLATFORMS[@]}"; do
    echo "  binaries/$platform/fuzzy"
done
