...
If you use an agent, run it from the `fuzzyos` root directory so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
...
If you're adding a new provider to `packages/fuzzy-ai`, see `AGENTS.md` for required tests.

## Philosophy

fuzzy's core is minimal. If your feature doesn't belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.
